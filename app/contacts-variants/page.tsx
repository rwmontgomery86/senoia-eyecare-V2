...
export const metadata = { title: "Contacts — Marlo Variants" };

const PREVIEW_WIDTH = 1440;
const SCALE = 0.34;
const CARD_HEIGHT = 360;

export default function ContactsVariantsIndex() {
  return (
        ...
        <header className="mb-12 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <span className="font-mono text-[11px] uppercase tracking-eyebrow text-accent">
              Sandbox · Contacts — Marlo band
            </span>
            <h1 className="mt-3 font-display text-[clamp(2.5rem,5vw,3.75rem)] font-medium leading-[1]">
              Reorder band — <em className="text-accent">four directions.</em>
            </h1>
            <p className="mt-4 max-w-[640px] text-[15px] leading-[1.55] text-muted">
              A slim wayfinder for existing patients to reorder on Marlo. Click
              any variant to see it full-width with prev / next nav. Layouts
              only — animations and final polish come after picking a
              direction.
            </p>
          </div>
          <Link
