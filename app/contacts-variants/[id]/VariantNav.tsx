...
export default function VariantNav({
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft") {
        router.push(`/contacts-variants/${prevId}`);
      } else if (e.key === "ArrowRight") {
        router.push(`/contacts-variants/${nextId}`);
      } else if (e.key === "Escape") {
        router.push("/contacts-variants");
      }
    };
