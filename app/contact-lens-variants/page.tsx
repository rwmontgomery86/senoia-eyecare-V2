...
export const metadata = { title: "Contact-Lens Brands — Variants" };

const PREVIEW_WIDTH = 1440;
const SCALE = 0.34;
const CARD_HEIGHT = 540;

export default function ContactLensVariantsIndex() {
  return (
        ...
        <header className="mb-12 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <span className="font-mono text-[11px] uppercase tracking-eyebrow text-accent">
              Sandbox · Contact-Lens Brands
            </span>
            <h1 className="mt-3 font-display text-[clamp(2.5rem,5vw,3.75rem)] font-medium leading-[1]">
              Brands section — <em className="text-accent">three directions.</em>
            </h1>
            <p className="mt-4 max-w-[640px] text-[15px] leading-[1.55] text-muted">
              Three macro layouts spanning minimal → editorial → rich. Click any
              variant to see it full-width with prev / next nav. Layouts only —
              animations and final polish come after picking a direction.
            </p>
          </div>
          <Link
            href="/contacts"
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-3">
          {variants.map((v) => {
            ...
            return (
              <Link
                key={v.id}
                href={`/contact-lens-variants/${v.id}`}
