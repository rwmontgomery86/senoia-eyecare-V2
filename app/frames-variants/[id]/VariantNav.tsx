...
export default function VariantNav({
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft") {
        router.push(`/frames-variants/${prevId}`);
      } else if (e.key === "ArrowRight") {
        router.push(`/frames-variants/${nextId}`);
      } else if (e.key === "Escape") {
        router.push("/frames-variants");
      }
    };
