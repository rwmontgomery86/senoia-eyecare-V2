...
import { variants } from "@/components/sections/made-for-variants";

export const metadata = { title: "Made For — Variants" };
...
export default function MadeForVariantsIndex() {
  return (
        ...
        <header className="mb-12 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-3 font-display text-[clamp(2.5rem,5vw,3.75rem)] font-medium leading-[1]">
              Made For section — <em className="text-accent">three directions.</em>
            </h1>
            <p className="mt-4 max-w-[640px] text-[15px] leading-[1.55] text-muted">
              Click any variant to see it full-width with prev / next nav.
              All three render the same content (cycling) so the comparison is
              purely about layout.
            </p>
          </div>
          <Link
          ...
          {variants.map((v) => {
            ...
            return (
              ...
              >
                ...
                {/* Overlay link — covers the whole card, sits above the preview so nested anchors inside the variant don't break HTML */}
                <Link
                  href={`/made-for-variants/${v.id}`}
