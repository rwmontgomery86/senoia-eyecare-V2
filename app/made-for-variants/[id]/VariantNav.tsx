...
export default function VariantNav({
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft") {
        router.push(`/made-for-variants/${prevId}`);
      } else if (e.key === "ArrowRight") {
        router.push(`/made-for-variants/${nextId}`);
      } else if (e.key === "Escape") {
        router.push("/made-for-variants");
      }
    };
