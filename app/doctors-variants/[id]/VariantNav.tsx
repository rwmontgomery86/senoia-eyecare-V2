...
export default function VariantNav({
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft") {
        router.push(`/doctors-variants/${prevId}`);
      } else if (e.key === "ArrowRight") {
        router.push(`/doctors-variants/${nextId}`);
      } else if (e.key === "Escape") {
        router.push("/doctors-variants");
      }
    };
